...
/** Base action for copying the already displayed Code Issues occurrences. */
abstract class AbstractCopyInspectionResultsAction(
    private val panel: JComponent,
    private val tree: JTree,
    text: String,
    description: String
) : DumbAwareAction(text, description, AllIcons.Actions.Copy) {
    ...
    protected open fun formatPayload(payload: String): String = payload
    ...
    private fun subscribe(model: Any) {
        ...
        typedModel.result.advise(Lifetime.Eternal) { report ->
            ...
            if (waiting.compareAndSet(true, false)) {
                val formatted = formatPayload(payload)
                log.info("Received inspection-copy result: ${formatted.length} characters")
